<template>
    ...
    >
        <svg
            class="select-none w-full overflow-visible block aspect-[100/12] text-2xl !text-[#66033C] leading-none"
        ...
        >
            ...
            <text v-if="ready" xml:space="preserve" :class="['', className]">
                <textPath
                    ...
                    xml:space="preserve"
                >
                    <template v-for="r in repeatCount" :key="r">
                        <tspan v-for="(w, wi) in words" :key="`${r}-${wi}`">
                            {{ w }}
                            <tspan fill="#FFE8EE">●</tspan>
                        </tspan>
                    </template>
                </textPath>
            </text>
...
const words = computed(() => {
    const raw = (props.marqueeText || '').trim();
    if (!raw) return [];
    return raw.split(/\s+/);
});

const measured = computed(() => {
    // join words with the same separator used in rendering (a bullet)
    if (!words.value.length) return '';
    return words.value.join('\u25CF') + '\u00A0';
});

const repeatCount = computed(() => {
    if (!spacing.value) return 1;
    return Math.ceil(1800 / spacing.value) + 2;
});
